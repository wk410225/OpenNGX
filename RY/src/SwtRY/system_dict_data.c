...
static TABFUN SYS_DICT_DATA_COLMAP[MAXCOLNUM]={
    ...
    {"dict_label", offsetof(sys_dict_data_def, dict_label), 'C', "dictLabel", "dictLabel"},
...
int system_dict_data(char *sMsgBuf, int nIndex )
{
	char			sFuncName[] = "system_dict_data";
	...
	if (strcmp(sMethod, "DELETE" ) == 0  && memcmp(sUri, "/system/dict/data/", strlen("/system/dict/data/")) == 0 && atoi(param) > 0) 
	{
		memset(sSql,  0x00, sizeof(sSql));
		sprintf(sSql, "delete from sys_dict_data where dict_code= %s", param);
...
*/
	...
	else if (strcmp(sMethod, "DELETE" ) == 0  && strcmp(sUri, "/system/dict/data/refreshCache") == 0 ) 
	{
        root = cJSON_CreateObject();
        cJSON_AddStringToObject(root, "msg", "操作成功");
        cJSON_AddNumberToObject(root, "code", 200);
	...
	else if (strcmp(sMethod, "GET" ) == 0  && memcmp(sUri, "/system/dict/data/", strlen("/system/dict/data/")) == 0 && atoi(param) > 0) 
	{
    	sys_dict_data_def    ptTbl;
		...
		memset(sSql,  0x00, sizeof(sSql));
        sprintf(sSql, "select * from sys_dict_data where dict_code = %s", param);
		...
		while(1)
		{
			...
			nRet = mysql_TBL_FETCH(NULL, &ptTbl, SYS_DICT_DATA_COLMAP);
			if(nRet == 1403)
			{
			...
			else
			{
                ...
                NS2J(pdata, &ptTbl, SYS_DICT_DATA_COLMAP);
	...
	else if (strcmp(sMethod, "GET" ) == 0  && strcmp(sUri, "/system/dict/data/list") == 0) //菜单列表
	{
		sys_dict_data_def	ptTbl;
		...
		memset(sSql,  0x00, sizeof(sSql));
		sprintf(sSql, "select count(1) as total_rows from sys_dict_data ");
		NGetQueryWhere(IPC, sSql,  SYS_DICT_DATA_COLMAP);
		...
		memset(sSql,  0x00, sizeof(sSql));
        sprintf(sSql, "select * from sys_dict_data ");
		NGetQueryWhere(IPC, sSql,  SYS_DICT_DATA_COLMAP);
		HtLog (gsLogFile, HT_LOG_MODE_DEBUG, __FILE__,__LINE__, "%s", sSql);
		mysql_TBL_CURSOR(sSql, NULL);
		mysql_TBL_OPEN(sSql, NULL);
	...
	else if (strcmp(sMethod, "PUT" ) == 0  && strcmp(sUri, "/system/dict/data") == 0) //修改
	{
		TABFUN SYS_DICT_DATA_INDMAP[MAXCOLNUM]={
			{"dict_code", offsetof(sys_dict_data_def, dict_code), 'I', "dictCode",  "",""},
		};
        ...
        memset((char *)&ptTbl, 0x00, sizeof(ptTbl));
        NJ2S(root, &ptTbl, SYS_DICT_DATA_COLMAP);
		GetCurrTime(ptTbl.update_time);
        PrintTbl(&ptTbl, SYS_DICT_DATA_COLMAP);
        memset(sSql,  0x00, sizeof(sSql));
        memset(set, 0x00, sizeof(set));
        memset(where, 0x00, sizeof(where));
        NGetUpdateSetSql(root, set, SYS_DICT_DATA_COLMAP);
        NGetUpdateWhereSql(root, where, SYS_DICT_DATA_INDMAP);
        sprintf(sSql, "update sys_dict_data  set %s where %s;", set, where);
	...
	else if (strcmp(sMethod, "POST" ) == 0  && strcmp(sUri, "/system/dict/data/export") == 0) //导出
	{
		xlsxiowriter handle;
		if ((handle = xlsxiowrite_open("dictdata.xlsx", "字典明细")) == NULL) 
		{
		...
		xlsxiowrite_add_column( handle,"字典数据",  0);
		...
		parray = cJSON_CreateArray();
		memset(sSql,  0x00, sizeof(sSql));
		strcpy(sSql, "select * from sys_dict_data ");
		NGetQueryWhere(IPC, sSql,  SYS_DICT_DATA_COLMAP);
		...
		while(1)
		{
			...
			{
				PrintTbl(&ptTbl, SYS_DICT_DATA_COLMAP);
				xlsxiowrite_add_cell_int(handle,      ptTbl.dict_code);
				xlsxiowrite_add_cell_string(handle,   ptTbl.dict_label);
				xlsxiowrite_add_cell_string(handle,   ptTbl.dict_type);
				xlsxiowrite_add_cell_string(handle,   ptTbl.dict_value);
		...
		FILE *file = fopen("dictdata.xlsx", "rb");
	...
	else if (strcmp(sMethod, "POST" ) == 0  && strcmp(sUri, "/system/dict/data") == 0) //新增状态
	{
        root = cJSON_Parse(json_string);
        if (root == NULL) {
            const char *error_ptr = cJSON_GetErrorPtr();
        ...
        PrintTbl(&ptTbl, SYS_DICT_DATA_COLMAP);
		HtLog(gsLogFile, HT_LOG_MODE_ERROR, __FILE__,__LINE__, "update sql=[%s]", sSql);
		nRet = mysql_TBL_INSERT("sys_dict_data", &ptTbl, SYS_DICT_DATA_COLMAP);
		if(nRet != 0)
		{
			HtLog(gsLogFile, HT_LOG_MODE_ERROR, __FILE__,__LINE__, "update error [%s]", sSql);
			return -1;
