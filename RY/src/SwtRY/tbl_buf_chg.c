...
static TABFUN TBL_BUF_CHG_COLMAP[MAXCOLNUM]={
    {"l_usage_key", offsetof(tbl_buf_chg_def, l_usage_key), 'I'},
    {"i_buf_chg_index", offsetof(tbl_buf_chg_def, i_buf_chg_index), 'I'},
    {"i_sour_buf_index", offsetof(tbl_buf_chg_def, i_sour_buf_index), 'I'},
    {"i_dest_buf_index", offsetof(tbl_buf_chg_def, i_dest_buf_index), 'I'},
    {"sp_dsp", offsetof(tbl_buf_chg_def, sp_dsp), 'C'},
};


static TABFUN TBL_BUF_CHG_INDMAP[MAXINDNUM]={
    {"l_usage_key", offsetof(tbl_buf_chg_def, l_usage_key), 'I'},
    {"i_buf_chg_index", offsetof(tbl_buf_chg_def, i_buf_chg_index), 'I'},
};

int tbl_buf_chg(char *sMsgBuf, int nIndex )
{
	char			sFuncName[] = "tbl_buf_chg";
	...
	if (strcmp(sMethod, "POST" ) == 0 )
	{
		...
		J2S(root, &ptCurrentTbl, TBL_BUF_CHG_COLMAP);
		PrintTbl(&ptCurrentTbl, TBL_BUF_CHG_COLMAP);
		nRet = mysql_TBL_INSERT("tbl_buf_chg", &ptCurrentTbl, TBL_BUF_CHG_COLMAP);
		if(nRet != 0)
		{
	...
	else if (strcmp(sMethod, "GET" ) == 0 )
	{
		...
		if( pageNum == 0 || pageSize == 0 )
		{
			...
			sprintf(sSql, "select count(1) as total_rows from tbl_buf_chg ");
			GetQueryWhere(IPC, sSql,  TBL_BUF_CHG_COLMAP);
		...
		else
		{
			...
			memset(sSql,  0x00, sizeof(sSql));
			strcpy(sSql, "select * from tbl_buf_chg ");
			GetQueryWhere(IPC, sSql,  TBL_BUF_CHG_COLMAP);
			...
			{
				memset((char *)&ptCurrentTbl, 0x00, sizeof(ptCurrentTbl));
				nRet = mysql_TBL_FETCH(NULL, &ptCurrentTbl, TBL_BUF_CHG_COLMAP);
				if(nRet == 1403)
					break;
				else if(nRet == -1)
				{
				...
				else
				{
					...
					S2J(item, &ptCurrentTbl, TBL_BUF_CHG_COLMAP);
	...
	else if (strcmp(sMethod, "PUT" ) == 0 )
    {
		...
		GetUpdateSetSql(root, set, TBL_BUF_CHG_COLMAP);
		GetUpdateWhereSql(root, where, TBL_BUF_CHG_INDMAP);
		sprintf(sSql, "update tbl_buf_chg  set %s where %s;", set, where);
	...
	else if (strcmp(sMethod, "DELETE" ) == 0 )
    {
		...
		GetDeleteWhereSql(root, where, TBL_BUF_CHG_INDMAP);
		/*must delete depended  on unique index column */
		sprintf(sSql, "delete from  tbl_buf_chg  where %s;",  where);
		if (GetDeleteWhereSql(root, where , TBL_BUF_CHG_INDMAP) < 0)
		{
