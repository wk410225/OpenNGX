...
static TABFUN TBL_BUF_DSP_COLMAP[MAXCOLNUM]={
	{"l_usage_key", offsetof(tbl_buf_dsp_def, l_usage_key), 'I'},
    {"i_buf_dsp_index", offsetof(tbl_buf_dsp_def, i_buf_dsp_index), 'I'},
    {"i_pos_index", offsetof(tbl_buf_dsp_def, i_pos_index), 'I'},
    {"i_fld_index", offsetof(tbl_buf_dsp_def, i_fld_index), 'I'},
    {"i_fld_id", offsetof(tbl_buf_dsp_def, i_fld_id), 'I'},
    {"i_fld_offset", offsetof(tbl_buf_dsp_def, i_fld_offset), 'I'},


};


static TABFUN TBL_BUF_DSP_INDMAP[MAXINDNUM]={
	{"l_usage_key", offsetof(tbl_buf_dsp_def, l_usage_key), 'I'},
    {"i_buf_dsp_index", offsetof(tbl_buf_dsp_def, i_buf_dsp_index), 'I'},
    {"i_pos_index", offsetof(tbl_buf_dsp_def, i_pos_index), 'I'},
};

int tbl_buf_dsp(char *sMsgBuf, int nIndex )
{
	char			sFuncName[] = "tbl_buf_dsp";
	...
	if (strcmp(sMethod, "POST" ) == 0 )
	{
		...
		J2S(root, &ptCurrentTbl, TBL_BUF_DSP_COLMAP);
		PrintTbl(&ptCurrentTbl, TBL_BUF_DSP_COLMAP);
		nRet = mysql_TBL_INSERT("tbl_buf_dsp", &ptCurrentTbl, TBL_BUF_DSP_COLMAP);
		if(nRet != 0)
		{
...
/*
		...
		if( pageNum == 0 || pageSize == 0 )
		{
			...
			sprintf(sSql, "select count(1) as total_rows from tbl_buf_dsp ");
			GetQueryWhere(IPC, sSql,  TBL_BUF_DSP_COLMAP);
...
*/
		if(GetIpcValueExt(IPC, "$reserve", sTmpBuf, sizeof(sTmpBuf)) > 0)
		{
			if(strcmp(sTmpBuf, "tbl_fld_tsf") == 0)
			{
				...
				memset(sSql,  0x00, sizeof(sSql));
				strcpy(sSql, "select * from tbl_buf_dsp a left join  tbl_fld_tsf b on a.i_pos_index = b.i_sour_fld_index ");
				GetQueryWhereSchema(IPC, sSql,  TBL_BUF_DSP_COLMAP, "a");
				...
				{
					memset((char *)&ptCurrentTbl, 0x00, sizeof(ptCurrentTbl));
					nRet = mysql_TBL_FETCH(NULL, &ptCurrentTbl, TBL_BUF_DSP_COLMAP);
					if(nRet == 1403)
						break;
					else if(nRet == -1)
					{
					...
					else
					{
						...
						S2J(item, &ptCurrentTbl, TBL_BUF_DSP_COLMAP);
		...
		else
		{
			array = cJSON_CreateArray();
			memset(sSql,  0x00, sizeof(sSql));
			strcpy(sSql, "select * from tbl_buf_dsp ");
			GetQueryWhere(IPC, sSql,  TBL_BUF_DSP_COLMAP);
			GetQuerySortLimit(IPC, sSql );
			mysql_TBL_CURSOR(sSql, NULL);
			mysql_TBL_OPEN(sSql, NULL);
	...
	else if (strcmp(sMethod, "PUT" ) == 0 )
    {
		...
		GetUpdateSetSql(root, set, TBL_BUF_DSP_COLMAP);
		GetUpdateWhereSql(root, where, TBL_BUF_DSP_INDMAP);
		sprintf(sSql, "update tbl_buf_dsp  set %s where %s;", set, where);
	...
	else if (strcmp(sMethod, "DELETE" ) == 0 )
    {
		...
		GetDeleteWhereSql(root, where, TBL_BUF_DSP_INDMAP);
		/*must delete depended  on unique index column */
		sprintf(sSql, "delete from  tbl_buf_dsp  where %s;",  where);
		if (GetDeleteWhereSql(root, where , TBL_BUF_DSP_INDMAP) < 0)
		{
