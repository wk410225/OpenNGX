...
static TABFUN SYS_CONFIG_COLMAP[MAXCOLNUM]={
	{"config_id",    offsetof(sys_config_def, config_id      ), 'I', "configId",    "configId",    ""},
	{"config_name",  offsetof(sys_config_def, config_name    ), 'C', "configName",  "configName",  ""},
	{"config_key",   offsetof(sys_config_def, config_key     ), 'C', "configKey",   "configKey",   ""},
	{"config_value", offsetof(sys_config_def, config_value   ), 'C', "configValue", "configValue", ""},
	{"config_type",  offsetof(sys_config_def, config_type    ), 'C', "configType",  "configType",  ""},
	{"create_by",    offsetof(sys_config_def, create_by      ), 'C', "createBy",    "createBy",    ""},
	{"create_time",  offsetof(sys_config_def, create_time    ), 'T', "params[beginTime]",  "createTime",  "params[endTime]"},
	{"update_by",    offsetof(sys_config_def, update_by      ), 'C', "updateBy",    "updateBy",    ""},
	{"update_time",  offsetof(sys_config_def, update_time    ), 'T', "updateTime",  "updateTime",  ""},
	{"remark",       offsetof(sys_config_def, remark         ), 'C', "remark",       "remark",       ""},
};



int system_config(char *sMsgBuf, int nIndex )
{
	char			sFuncName[] = "system_config";
	...
	if (strcmp(sMethod, "DELETE" ) == 0 && strcmp(sUri, "/system/config/refreshCache") == 0) // 删除用户
	{
        root = cJSON_CreateObject();
	...
	else if (strcmp(sMethod, "DELETE" ) == 0 && atoi(param) > 0 ) // 删除用户
	{
		...
		sprintf(sSql, "delete from sys_config where config_id= %s", param);
	...
	else if (strcmp(sMethod, "GET" ) == 0  && memcmp(sUri, "/system/config/", strlen("/system/config/")) == 0 && atoi(param) > 0) 
	{
    	sys_config_def    ptTblMenu;
		...
		memset(sSql,  0x00, sizeof(sSql));
        sprintf(sSql, "select * from sys_config where config_id = %s", param);
		...
		while(1)
		{
			...
			nRet = mysql_TBL_FETCH(NULL, &ptTblMenu, SYS_CONFIG_COLMAP);
			if(nRet == 1403)
			{
			...
			else
			{
                ...
                NS2J(pdata, &ptTblMenu, SYS_CONFIG_COLMAP);
	...
	else if (strcmp(sMethod, "GET" ) == 0  && memcmp(sUri, "/system/config/configKey/", strlen("/system/config/configKey/")) == 0) 
	{
		root = cJSON_CreateObject();
        memset(sSql,  0x00, sizeof(sSql));
        sprintf(sSql, "select * from sys_config where  config_key='%s'", param);
        memset((char*)&ptTbl, 0x00, sizeof(ptTbl));
        nRet = mysql_ONE_SELECT(sSql, &ptTbl, SYS_CONFIG_COLMAP);
        if(nRet !=0 ) {
            HtLog (gsLogFile, HT_LOG_MODE_NORMAL, __FILE__,__LINE__, "error %s", sSql);
            return -1;
        }
        cJSON_AddNumberToObject(root, "code", 200);
        NS2J(root, &ptTbl, SYS_CONFIG_COLMAP);
        json_string = cJSON_Print(root);
        HtLog(gsLogFile, HT_LOG_MODE_DEBUG, __FILE__,__LINE__, "json_string[%s]", cJSON_Print(root));
	}

	else if (strcmp(sMethod, "GET" ) == 0  && strcmp(sUri, "/system/config/list") == 0) //菜单列表
	{
    	sys_config_def    ptTblMenu;
        ...
        NGetQueryWhere(IPC, sSql,  SYS_CONFIG_COLMAP);
		...
		memset(sSql,  0x00, sizeof(sSql));
        sprintf(sSql, "select * from sys_config ");
		NGetQueryWhere(IPC, sSql,  SYS_CONFIG_COLMAP);
		strcat(sSql, " order by config_id asc");
		HtLog (gsLogFile, HT_LOG_MODE_DEBUG, __FILE__,__LINE__, "%s", sSql);
		mysql_TBL_CURSOR(sSql, NULL);
		mysql_TBL_OPEN(sSql, NULL);
		parray= cJSON_CreateArray();
	...
	else if (strcmp(sMethod, "PUT" ) == 0  && strcmp(sUri, "/system/config") == 0) //修改状态
	{
		TABFUN SYS_CONFIG_INDMAP[MAXCOLNUM]={
			{"config_id", offsetof(sys_config_def, config_id), 'I', "configId",  "",""},
		};
		sys_config_def	ptTbl;
        ...
        memset((char *)&ptTbl, 0x00, sizeof(ptTbl));
        NJ2S(root, &ptTbl, SYS_CONFIG_COLMAP);
		GetCurrTime(ptTbl.update_time);
        PrintTbl(&ptTbl, SYS_CONFIG_COLMAP);
        memset(sSql,  0x00, sizeof(sSql));
        memset(set, 0x00, sizeof(set));
        memset(where, 0x00, sizeof(where));
        NGetUpdateSetSql(root, set, SYS_CONFIG_COLMAP);
        NGetUpdateWhereSql(root, where, SYS_CONFIG_INDMAP);
	...
	else if (strcmp(sMethod, "POST" ) == 0  && strcmp(sUri, "/system/config") == 0) //新增状态
	{
		sys_config_def	ptTbl;
        root = cJSON_Parse(json_string);
        if (root == NULL) {
        ...
        PrintTbl(&ptTbl, SYS_CONFIG_COLMAP);
		HtLog(gsLogFile, HT_LOG_MODE_ERROR, __FILE__,__LINE__, "update sql=[%s]", sSql);
		nRet = mysql_TBL_INSERT("sys_config", &ptTbl, SYS_CONFIG_COLMAP);
		if(nRet != 0)
		{
			HtLog(gsLogFile, HT_LOG_MODE_ERROR, __FILE__,__LINE__, "update error [%s]", sSql);
			return -1;
	...
	else if (strcmp(sMethod, "POST" ) == 0  && strcmp(sUri, "/system/config/export") == 0) //角色导出
	{
		xlsxiowriter handle;
		if ((handle = xlsxiowrite_open("post.xlsx", "岗位数据")) == NULL) 
		{
		...
		parray = cJSON_CreateArray();
		memset(sSql,  0x00, sizeof(sSql));
		strcpy(sSql, "select * from sys_config ");
		NGetQueryWhere(IPC, sSql,  SYS_CONFIG_COLMAP);
		...
		{
			memset((char *)&ptTbl, 0x00, sizeof(ptTbl));
			nRet = mysql_TBL_FETCH(NULL, &ptTbl, SYS_CONFIG_COLMAP);
			if(nRet == 1403)
			{
				mysql_TBL_CLOSE(sSql, NULL);
				break;
			...
			{
				PrintTbl(&ptTbl, SYS_CONFIG_COLMAP);
				xlsxiowrite_add_cell_int(handle, ptTbl.config_id);
				xlsxiowrite_add_cell_string(handle,   ptTbl.config_name);
				xlsxiowrite_add_cell_string(handle,   ptTbl.config_key);
				xlsxiowrite_add_cell_string(handle,   ptTbl.config_value);
				xlsxiowrite_next_row(handle);
			}
		}
		...
		SetIpcValue(sSql, "$VAL1", "attachment; filename=post.xlsx");
