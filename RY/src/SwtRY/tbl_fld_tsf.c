...
static TABFUN TBL_FLD_TSF_COLMAP[MAXCOLNUM]={
    {"l_usage_key", offsetof(tbl_fld_tsf_def, l_usage_key), 'I'},
    {"i_buf_chg_index", offsetof(tbl_fld_tsf_def, i_buf_chg_index), 'I'},
    {"i_sour_fld_index", offsetof(tbl_fld_tsf_def, i_sour_fld_index), 'I'},
    {"i_dest_fld_index", offsetof(tbl_fld_tsf_def, i_dest_fld_index), 'I'},
};

static TABFUN TBL_FLD_TSF_INDMAP[MAXINDNUM]={
    {"l_usage_key", offsetof(tbl_fld_tsf_def, l_usage_key), 'I'},
    {"i_buf_chg_index", offsetof(tbl_fld_tsf_def, i_buf_chg_index), 'I'},
    {"i_sour_fld_index", offsetof(tbl_fld_tsf_def, i_sour_fld_index), 'I'},
    {"i_dest_fld_index", offsetof(tbl_fld_tsf_def, i_dest_fld_index), 'I'},
...
int tbl_fld_tsf(char *sMsgBuf, int nIndex )
{
	char			sFuncName[] = "tbl_fld_tsf";
	...
	if (strcmp(sMethod, "POST" ) == 0 )
	{
		...
		J2S(root, &ptCurrentTbl, TBL_FLD_TSF_COLMAP);
		PrintTbl(&ptCurrentTbl, TBL_FLD_TSF_COLMAP);
		nRet = mysql_TBL_INSERT("tbl_fld_tsf", &ptCurrentTbl, TBL_FLD_TSF_COLMAP);
		if(nRet != 0)
		{
...
/*
		...
		if( pageNum == 0 || pageSize == 0 )
		{
			...
			sprintf(sSql, "select count(1) as total_rows from tbl_fld_tsf ");
			GetQueryWhere(IPC, sSql,  TBL_FLD_TSF_COLMAP);
...
*/
		{
			...
			memset(sSql,  0x00, sizeof(sSql));
			strcpy(sSql, "select * from tbl_fld_tsf ");
			GetQueryWhere(IPC, sSql,  TBL_FLD_TSF_COLMAP);
			...
			{
				memset((char *)&ptCurrentTbl, 0x00, sizeof(ptCurrentTbl));
				nRet = mysql_TBL_FETCH(NULL, &ptCurrentTbl, TBL_FLD_TSF_COLMAP);
				if(nRet == 1403)
					break;
				else if(nRet == -1)
				{
				...
				else
				{
					...
					S2J(item, &ptCurrentTbl, TBL_FLD_TSF_COLMAP);
	...
	else if (strcmp(sMethod, "PUT" ) == 0 )
    {
		...
		GetUpdateSetSql(root, set, TBL_FLD_TSF_COLMAP);
		GetUpdateWhereSql(root, where, TBL_FLD_TSF_INDMAP);
		sprintf(sSql, "update tbl_fld_tsf  set %s where %s;", set, where);
	...
	else if (strcmp(sMethod, "DELETE" ) == 0 )
    {
		...
		GetDeleteWhereSql(root, where, TBL_FLD_TSF_INDMAP);
		/*must delete depended  on unique index column */
		sprintf(sSql, "delete from  tbl_fld_tsf  where %s;",  where);
		if (GetDeleteWhereSql(root, where , TBL_FLD_TSF_INDMAP) < 0)
		{
