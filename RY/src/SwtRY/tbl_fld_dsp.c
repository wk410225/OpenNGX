...
static TABFUN TBL_FLD_DSP_COLMAP[MAXCOLNUM]={
    {"l_usage_key", offsetof(tbl_fld_dsp_def, l_usage_key), 'I'},
    {"i_fld_id", offsetof(tbl_fld_dsp_def, i_fld_id), 'I'},
    {"n_fld_l", offsetof(tbl_fld_dsp_def, n_fld_l), 'I'},
    {"n_fld_type", offsetof(tbl_fld_dsp_def, n_fld_type), 'I'},
    {"sp_fld_dsp", offsetof(tbl_fld_dsp_def, sp_fld_dsp), 'C'},
};

static TABFUN TBL_FLD_DSP_INDMAP[MAXINDNUM]={
    {"l_usage_key", offsetof(tbl_fld_dsp_def, l_usage_key), 'I'},
    {"i_fld_id", offsetof(tbl_fld_dsp_def, i_fld_id), 'I'},
};

int tbl_fld_dsp(char *sMsgBuf, int nIndex )
{
	char			sFuncName[] = "tbl_fld_dsp";
	...
	if (strcmp(sMethod, "POST" ) == 0 )
	{
		...
		J2S(root, &ptCurrentTbl, TBL_FLD_DSP_COLMAP);
		PrintTbl(&ptCurrentTbl, TBL_FLD_DSP_COLMAP);
		nRet = mysql_TBL_INSERT("tbl_fld_dsp", &ptCurrentTbl, TBL_FLD_DSP_COLMAP);
		if(nRet != 0)
		{
...
/*
		...
		if( pageNum == 0 || pageSize == 0 )
		{
			...
			sprintf(sSql, "select count(1) as total_rows from tbl_fld_dsp ");
			GetQueryWhere(IPC, sSql,  TBL_FLD_DSP_COLMAP);
...
*/
		{
			...
			memset(sSql,  0x00, sizeof(sSql));
			strcpy(sSql, "select * from tbl_fld_dsp ");
			GetQueryWhere(IPC, sSql,  TBL_FLD_DSP_COLMAP);
			...
			{
				memset((char *)&ptCurrentTbl, 0x00, sizeof(ptCurrentTbl));
				nRet = mysql_TBL_FETCH(NULL, &ptCurrentTbl, TBL_FLD_DSP_COLMAP);
				if(nRet == 1403)
					break;
				else if(nRet == -1)
				{
				...
				else
				{
					...
					S2J(item, &ptCurrentTbl, TBL_FLD_DSP_COLMAP);
	...
	else if (strcmp(sMethod, "PUT" ) == 0 )
    {
		...
		GetUpdateSetSql(root, set, TBL_FLD_DSP_COLMAP);
		GetUpdateWhereSql(root, where, TBL_FLD_DSP_INDMAP);
		sprintf(sSql, "update tbl_fld_dsp  set %s where %s;", set, where);
	...
	else if (strcmp(sMethod, "DELETE" ) == 0 )
    {
		...
		GetDeleteWhereSql(root, where, TBL_FLD_DSP_INDMAP);
		/*must delete depended  on unique index column */
		sprintf(sSql, "delete from  tbl_fld_dsp  where %s;",  where);
		if (GetDeleteWhereSql(root, where , TBL_FLD_DSP_INDMAP) < 0)
		{
