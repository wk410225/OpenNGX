...
static TABFUN SYS_POST_COLMAP[MAXCOLNUM]={
	{"post_id",      offsetof(sys_post_def, post_id        ), 'I', "postId",      "postId",      ""},
	{"post_code",    offsetof(sys_post_def, post_code      ), 'C', "postCode",    "postCode",    ""},
	{"post_name",    offsetof(sys_post_def, post_name      ), 'C', "postName",    "postName",    ""},
	{"post_sort",    offsetof(sys_post_def, post_sort      ), 'I', "postSort",    "postSort",    ""},
	{"status",       offsetof(sys_post_def, status         ), 'C', "status",       "status",       ""},
	{"create_by",    offsetof(sys_post_def, create_by      ), 'C', "createBy",    "createBy",    ""},
	{"create_time",  offsetof(sys_post_def, create_time    ), 'T', "createTime",  "createTime",  ""},
	{"update_by",    offsetof(sys_post_def, update_by      ), 'C', "updateBy",    "updateBy",    ""},
	{"update_time",  offsetof(sys_post_def, update_time    ), 'T', "updateTime",  "updateTime",  ""},
	{"remark",       offsetof(sys_post_def, remark         ), 'C', "remark",       "remark",       ""},
};


int system_post(char *sMsgBuf, int nIndex )
{
	char			sFuncName[] = "system_post";
	...
	if (strcmp(sMethod, "DELETE" ) == 0  ) // 删除用户
	{
		...
		sprintf(sSql, "delete from sys_post where post_id= %s", param);
		...
		memset(sSql,  0x00, sizeof(sSql));
		sprintf(sSql, "delete from sys_user_post where post_id= %s", param);
        HtLog(gsLogFile, HT_LOG_MODE_NORMAL, __FILE__,__LINE__, "%s", sSql);
		nRet = mysql_TBL_DELETE(sSql);
        if(nRet != 0)
        {
	...
	else if (strcmp(sMethod, "GET" ) == 0  && memcmp(sUri, "/system/post/", strlen("/system/post/")) == 0 && atoi(param) > 0) 
	{
    	sys_post_def    ptTblMenu;
		...
		memset(sSql,  0x00, sizeof(sSql));
        sprintf(sSql, "select * from sys_post where post_id = %s", param);
		...
		while(1)
		{
			...
			nRet = mysql_TBL_FETCH(NULL, &ptTblMenu, SYS_POST_COLMAP);
			if(nRet == 1403)
			{
			...
			else
			{
                ...
                NS2J(pdata, &ptTblMenu, SYS_POST_COLMAP);
	...
	else if (strcmp(sMethod, "GET" ) == 0  && strcmp(sUri, "/system/post/list") == 0) //菜单列表
	{
    	sys_post_def    ptTblMenu;
		...
		memset(sSql,  0x00, sizeof(sSql));
        sprintf(sSql, "select count(1) as total_rows from sys_post ");
        NGetQueryWhere(IPC, sSql,  SYS_POST_COLMAP);
		...
		memset(sSql,  0x00, sizeof(sSql));
        sprintf(sSql, "select * from sys_post ");
		NGetQueryWhere(IPC, sSql,  SYS_POST_COLMAP);
		strcat(sSql, " order by post_sort asc");
		HtLog (gsLogFile, HT_LOG_MODE_DEBUG, __FILE__,__LINE__, "%s", sSql);
		mysql_TBL_CURSOR(sSql, NULL);
		mysql_TBL_OPEN(sSql, NULL);
		parray= cJSON_CreateArray();
	...
	else if (strcmp(sMethod, "PUT" ) == 0  && strcmp(sUri, "/system/post") == 0) //修改状态
	{
		TABFUN SYS_POST_INDMAP[MAXCOLNUM]={
			{"post_id", offsetof(sys_post_def, post_id), 'I', "postId",  "",""},
		};
		sys_post_def	ptTbl;
        ...
        NJ2S(root, &ptTbl, SYS_POST_COLMAP);
		GetCurrTime(ptTbl.update_time);
        PrintTbl(&ptTbl, SYS_POST_COLMAP);
        memset(sSql,  0x00, sizeof(sSql));
        memset(set, 0x00, sizeof(set));
        memset(where, 0x00, sizeof(where));
        NGetUpdateSetSql(root, set, SYS_POST_COLMAP);
        NGetUpdateWhereSql(root, where, SYS_POST_INDMAP);
        sprintf(sSql, "update sys_post  set %s where %s;", set, where);
	...
	else if (strcmp(sMethod, "POST" ) == 0  && strcmp(sUri, "/system/post") == 0) //新增状态
	{
		sys_post_def	ptTbl;
        root = cJSON_Parse(json_string);
        if (root == NULL) {
...
*/
		HtLog(gsLogFile, HT_LOG_MODE_ERROR, __FILE__,__LINE__, "update sql=[%s]", sSql);
		nRet = mysql_TBL_INSERT("sys_post", &ptTbl, SYS_POST_COLMAP);
		if(nRet != 0)
		{
			HtLog(gsLogFile, HT_LOG_MODE_ERROR, __FILE__,__LINE__, "update error [%s]", sSql);
			return -1;
	...
	else if (strcmp(sMethod, "POST" ) == 0  && strcmp(sUri, "/system/post/export") == 0) //角色导出
	{
		xlsxiowriter handle;
		...
		xlsxiowrite_set_detection_rows(handle, 5);
		//write column names
		...
		parray = cJSON_CreateArray();
		memset(sSql,  0x00, sizeof(sSql));
		strcpy(sSql, "select * from sys_post ");
		NGetQueryWhere(IPC, sSql,  SYS_POST_COLMAP);
		...
		while(1)
		{
			...
			{
				PrintTbl(&ptTblPost, SYS_POST_COLMAP);
				xlsxiowrite_add_cell_int(handle, ptTblPost.post_id);
				xlsxiowrite_add_cell_string(handle,   ptTblPost.post_code);
				xlsxiowrite_add_cell_string(handle,   ptTblPost.post_name);
				xlsxiowrite_add_cell_int(handle,   ptTblPost.post_sort);
				if(ptTblPost.status[0]=='0')
					xlsxiowrite_add_cell_string(handle,   "正常");
				else 
					xlsxiowrite_add_cell_string(handle,   "停用");
				xlsxiowrite_next_row(handle);
			}
		}
